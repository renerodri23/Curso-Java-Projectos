...
public class CategoriaRepositorioImpl implements Repositorio<Categoria>{
    ...
    public Categoria porId(long id) {
        ...
        try(PreparedStatement st = conn.prepareStatement("SELECT * FROM categoria WHERE idCategoria = ?")){
            st.setLong(1,id);
    ...
    public void eliminar(Long id) {
        try(PreparedStatement st = conn.prepareStatement("DELETE FROM categoria WHERE idCategoria=?")){
            st.setLong(1,id);
            st.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException(e);
